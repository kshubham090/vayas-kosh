...
Duration-matching alone does not equalize per-speaker clip counts. Elderly
...
A serious caveat applies specifically to IndicConformer and IndicWhisper, one we were not able to
fully resolve before running the audit. IndicVoices-R, this paper's evaluation corpus, is
...
has not published, and we do not make that framing (see Section~\ref{sec:discussion}). We raise
this as a general recommendation rather than a complaint specific to AI4Bharat, whose IndicVoices,
IndicVoices-R, and Vistaar releases are themselves valuable, well-documented contributions this
paper relies on directly: publishing a speaker- or utterance-level training manifest alongside
released ASR checkpoints, or at minimum a clear per-model statement of exactly which corpora
contributed training data, would let audits like this one resolve exactly this kind of question
rather than having to flag it as an open risk.
...
style rather than acoustic recognition failure, in the same direction this paper's central

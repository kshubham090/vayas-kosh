...
as suggestive, not confirmed. We release the full zero-shot audit pipeline, matched-cohort
sampling code, and per-clip results.
\end{abstract}
...
consequence we did not fully control for: no system in this audit is trained to emit disfluencies,
so any age-related difference in disfluency rate would itself produce a WER/CER gap attributable
to reference style, not acoustic recognition failure -- the same direction this paper's central
hypothesis (elderly speech is harder to recognize) would also predict. We did not have time to
re-run this analysis against \texttt{normalized} references before this draft; doing so, and
showing the effect direction holds under both reference styles, is the most direct way to
strengthen or overturn this paper's age-effect claims, and is the top item for follow-up work
...
\item \textbf{Gold-reference confound}: we score against \texttt{verbatim} transcripts, which
preserve disfluencies no audited system is trained to produce. Any elderly-control difference in
disfluency rate would inflate WER/CER gaps through reference style alone, in the same direction as
this paper's substantive hypothesis. We did not re-run the analysis against \texttt{normalized}
references before this draft; doing so is the highest-priority follow-up for confirming or
overturning this paper's age-effect claims.
